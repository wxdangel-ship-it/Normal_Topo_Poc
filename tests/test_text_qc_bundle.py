...
from normal_topo_poc.protocol.text_qc_bundle import build_demo_bundle, build_text_qc_bundle
from normal_topo_poc.utils.size_guard import MAX_BYTES, MAX_LINES, apply_size_limit, measure_text
...
def test_qc_bundle_format() -> None:
    ...
    assert "=== Normal_Topo_Poc TEXT_QC_BUNDLE v1 ===" in text
    assert "Project: Normal_Topo_Poc" in text
    assert "Run:" in text
...
def test_lint_blocks_oversize_lines() -> None:
    ...
    assert ok is False
    assert any(item.startswith("SIZE_LINES") for item in violations)
...
def test_lint_blocks_oversize_bytes() -> None:
    ...
    ok, violations = lint_text(text)
    assert ok is False
    assert any(item.startswith("SIZE_BYTES") for item in violations)
...
def test_demo_bundle_is_pasteable() -> None:
    ...
    assert not any(item.startswith("SIZE_") for item in violations)
