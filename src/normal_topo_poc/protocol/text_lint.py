...
from normal_topo_poc.utils.size_guard import MAX_BYTES, MAX_LINES, measure_text
...
def lint_text(text: str) -> tuple[bool, list[str]]:
    ...
    hard = [item for item in violations if item.code.startswith("SIZE_")]
    ok = len(hard) == 0
    return ok, [str(item) for item in violations]
